...
async function joinGame(event) {
  ...
  try {
    ...
    if (!gameExists) {
      ...
      setButtonLoading(
        joinBtn,
        '<i class="fas fa-sign-in-alt mr-2"></i>INTRĂ ÎN JOC',
        false
      );
      return;
    }
    ...
    if (
      ...
      showError(
        "Problemă de conexiune. Verifică internetul și încearcă din nou."
      );
...
async function handleControlButton() {
  ...
  const button = document.querySelector(
    '#continueGameForm button[data-mode="control"]'
  );
...
async function handleDisplayButton() {
  ...
  const gameCode = document.getElementById("continueGameCode").value.trim();
  const button = document.querySelector(
    '#continueGameForm button[data-mode="display"]'
  );
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    if (!connectionOk) {
      ...
      showError(
        "Problemă de conexiune la server. Unele funcții pot fi limitate."
      );
    } else {
