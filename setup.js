...
async function loadSets() {
  ...
  try {
    // Încărcăm DOAR seturile default pentru joc rapid
    const defaultSets = await window.supabaseClient.getQuestionSets("default");

    const setsWithCounts = await Promise.all(
      defaultSets.map(async (set) => {
        const setData = await window.supabaseClient.getQuestionSetWithQuestions(
          set.id
        );
        return {
          ...set,
...
async function loadQuestions() {
  ...
  try {
    const questions = await window.supabaseClient.getQuestions(
      selectedCategory
    );
...
function handlePreviewQuestionClick(e) {
  e.stopPropagation();
  showQuestionPreview(
    allQuestions.find((q) => q.id === this.dataset.questionId)
  );
}
  ...
  try {
    ...
    document.querySelectorAll(".preview-question-btn").forEach((btn) => {
      ...
      btn.addEventListener("click", function (e) {
        ...
        showQuestionPreview(
          questions.find((q) => q.id === this.dataset.questionId)
        );
      });
    });
  } catch (error) {
...
function setupSetPreviewListeners(setData) {
  document.querySelectorAll(".preview-question-btn").forEach((btn) => {
    btn.addEventListener("click", function (e) {
      ...
      const question = setData.questions.find(
        (q) => q.id === this.dataset.questionId
      );
